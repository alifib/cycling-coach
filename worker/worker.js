...
// Public, no CORS check (Companion fetches this server-side, not from a browser if (url.pathname.startsWith('/zwo/') && request.method === 'GET') {
...
return json({ error: 'proxy_error', message: String(err.message || err) }, allowed, ori
}
},
};
...
if (!env.ZWO_KV) {
return json({ error: 'kv_not_configured', message: 'ZWO_KV binding missing on Worker' },
}
const body = await request.json();
if (!body.xml || typeof body.xml !== 'string') {
return json({ error: 'bad_request', message: 'expected { xml, filename? }' }, allowed, or
}
